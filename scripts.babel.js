...
function availablePicture(currentBook) {
  ...
  if (currentBook.volumeInfo.imageLinks) {
    ...
    imageLinks = "css/unavailable-resized.png";
  }
  return imageLinks;
}
...
function savedBookFormat(bookId, savedTitle, savedAuthor, savedUrl, savedDescription) {
  let savedBookForm = '<div id="savedResult" >' + '<button type="button" id="trash" class="fa fa-trash" onclick="trashBook(\'' + bookId + '\')"></button>' + "<br>" + savedTitle + '<br>' + savedAuthor + '<br>' + '<img src=' + savedUrl + '/>' + '<p>' + savedDescription + '...' + '</p>' + '<br>' + '</div>';
...
function bookFormat(bookId, title, author, url, description, ISBN) {
  let bookForm = '<div id="bookResult">' + '<button type="button"  id= "bookmark"class="fa fa-bookmark" onclick="bookmarkBook(\'' + bookId + '\')"></button>' + "<h4 id='resultTitle'>" + title + '</h4>' + "<h5 id='resultAuthor'>" + author + '</h5>' + '<img src=' + url + ' />' + '<p>' + description + '</p>' + '<br>' + '<h5>' + 'ISBN:' + ISBN + '</h5>' + '</div>';
  return bookForm;
}
